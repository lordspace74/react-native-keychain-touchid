 ...
 */
public class BitGoAppLockImpl<T extends AppLockActivity> extends AppLock implements LifeCycleInterface {

    public static final String TAG = "BitGoAppLockImpl";

    /**
     * The {@link android.content.SharedPreferences} key used to store the password
     */
    private static final String PASSWORD_PREFERENCE_KEY = "BITGO_PASSCODE_PREFERENCE_KEY";
    /**
     * The {@link android.content.SharedPreferences} key used to store the token
     */
    private static final String TOKEN_PREFERENCE_KEY = "BITGO_TOKEN_PREFERENCE_KEY";
    /**
     * The {@link android.content.SharedPreferences} key used to store the initialization vector
     */
    private static final String VECTOR_PREFERENCE_KEY = "BITGO_INITIALIZATION_VECTOR_PREFERENCE_KEY";
    /**
    ...
    /**
     * Static instance of {@link BitGoAppLockImpl}
     */
    private static BitGoAppLockImpl mInstance;

    /**
     * Static method that allows to get back the current static Instance of {@link BitGoAppLockImpl}
     *
     * @param context       The current context of the {@link Activity}
     * @param activityClass The activity extending {@link AppLockActivity}
     * @return The instance.
     */
    public static BitGoAppLockImpl getInstance(Context context, Class<? extends AppLockActivity> activityClass) {
        synchronized (LockManager.class) {
            if (mInstance == null) {
                mInstance = new BitGoAppLockImpl<>(context, activityClass);
            }
        }
        return mInstance;
    }

    private BitGoAppLockImpl(Context context, Class<T> activityClass) {
        super();
    ...
    public String getToken() {
        return mSharedPreferences.getString(TOKEN_PREFERENCE_KEY, "");
    }

    public String getInitializationVector() {
        return mSharedPreferences.getString(VECTOR_PREFERENCE_KEY, "");
    }

    public boolean setInitializationVector(String token) {
        SharedPreferences.Editor editor = mSharedPreferences.edit();

        if (token == null) {
            editor.remove(VECTOR_PREFERENCE_KEY);
            editor.apply();
            this.disable();
        } else {
            editor.putString(VECTOR_PREFERENCE_KEY, token);
            editor.apply();
            this.enable();
        }
